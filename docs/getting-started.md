...
1. To run Thunderloop locally on your computer
    1. Create the TOML configuration directory: `sudo mkdir -p /etc/thunderbots`
    2. Create a TOML configuration file at `/etc/thunderbots/robot_config.toml` with the following content (replace values as needed):
        ```toml
        robot_id = "1"
        channel_id = "0"
        ...
        - `network_interface` is one of the interfaces listed by `ip a` (e.g. `wlan0`, `eth0`, etc.)
        - `channel_id` is the channel id of the robot (e.g. `0`, `1`, etc.)
        - `kick_coeff`, `kick_constant`, and `chip_pulse_width` are calibrated kicking parameters. When running locally, these can be set to `0`.
    3. Now, run Thunderloop with the following command:
