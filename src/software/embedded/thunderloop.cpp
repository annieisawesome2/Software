                          ...
                          TeamColour::YELLOW, robot_id_)
{
    ...
    LOG(INFO)
        << "THUNDERLOOP: to update Thunderloop configuration, edit TOML config file at "
        << TOML_CONFIG_FILE_PATH << " and restart Thunderloop";
}
    ...
    for (;;)
    {
        ...
        prev_iter_start_time = current_time;
        {
            ...
            *(robot_status_.mutable_primitive_executor_status()) =
            ...
            // Update TOML config with dynamic values
            {
                ZoneNamedN(_tracy_redis, "Thunderloop: Update TOML Config", true);

                toml_config_client_->setNoCommit(
                    ROBOT_BATTERY_VOLTAGE_CONFIG_KEY,
                    std::to_string(power_status_.battery_voltage()));
                toml_config_client_->setNoCommit(ROBOT_CURRENT_DRAW_CONFIG_KEY,
                                                  std::to_string(power_status_.current_draw()));
                // Flush every 10 iterations to reduce file I/O (approximately once per second at 60Hz)
                static int flush_counter = 0;
                if (++flush_counter >= 10)
                {
                    toml_config_client_->flush();
                    flush_counter = 0;
                }
            }
