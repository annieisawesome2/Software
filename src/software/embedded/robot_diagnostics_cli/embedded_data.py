...
class EmbeddedData:
    ...
    def __init__(self) -> None:
        ...
        # Use the constant from py_constants if available, otherwise default
        try:
            self.config_file_path = TOML_CONFIG_FILE_PATH
        except NameError:
            # Fallback if constant not available
            self.config_file_path = "/etc/thunderbots/robot_config.toml"
    ...
    def _get_value(self, key: str) -> str:
        """Get a value from TOML config, removing leading slash from key."""
        # Remove leading slash if present
        normalized_key = key.lstrip("/")
        value = self.config.get(normalized_key, "")
